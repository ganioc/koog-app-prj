...
module.exports = async (req, res) => {
  ...
  try{
    ...
    if (oldStatus !== user.status) {
      await setStatusAction('admin', req.body.username, 'status', oldStatus, user.status, session)
    }

    if (oldUnused !== user.unused) {
      await setStatusAction('admin', req.body.username, 'unused', oldUnused, user.unused, session)
    }
